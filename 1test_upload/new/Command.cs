...
{
    List<PropertyInfo> argsProperties =
        GetType()
            .GetProperties(
                BindingFlags.Public |
            ...
            .Where(x =>
                x.GetCustomAttribute(typeof(ArgumentAttribute)) is not null)
            .ToList();

    /*
     * Reject extra arguments that are not defined
     * by the command.
     */
    int maxArgumentPosition =
        argsProperties.Count == 0
            ? -1
            : argsProperties
                .Select(x =>
                    ((ArgumentAttribute)x.GetCustomAttribute(
                        typeof(ArgumentAttribute))!).Position)
                .Max();

    if (args.Length > maxArgumentPosition + 1)
        return "Invalid args length!";

    if (argsProperties
            .Where(x =>
                (((ArgumentAttribute)x.GetCustomAttribute(
                    typeof(ArgumentAttribute))!).Flags
                    & ArgumentFlags.Optional) != ArgumentFlags.Optional)
            .Count() > args.Length)
        return "Invalid args length!";
    ...
    foreach (var argProp in argsProperties)
    {
        ArgumentAttribute attr =
            (ArgumentAttribute)argProp.GetCustomAttribute(
                typeof(ArgumentAttribute))!;

        if (attr.Position + 1 > args.Length &&
