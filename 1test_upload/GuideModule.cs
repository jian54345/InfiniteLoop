    ...
    foreach (GuideGroupTable guide in GuideGroups.Value.Values)
{
    ...
    session.log.Info(
        $"[GuideAll] Add Guide: " +
        $"Id={guide.Id}, " +
        $"CompleteId={guide.CompleteId}, " +
        $"Ignore={guide.Ignore}, " +
        $"GuideType={guide.GuideType}, " +
        $"ConditionId={string.Join(",", guide.ConditionId ?? new List<int>())}");
}

session.OpenedGuideGroupId = null;

session.log.Info(
    $"[GuideAll] Total guides added: {addedGuideIds.Count}");
...
try
{
    ...
    session.log.Info(
        $"[GuideAll] Successfully saved {addedGuideIds.Count} guides.");
}
catch (Exception exception)
{
