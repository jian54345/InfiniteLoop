...
if defined GAME_EXE (
    "%PYTHON%" run_steam.py --with-mongo --mongod "%MONGOD%" --mongo-dbpath "%MONGO_DBPATH%" --no-ensure-account --launch-cmd "%GAME_EXE%"
) else (
    "%PYTHON%" run_steam.py --with-mongo --mongod "%MONGOD%" --mongo-dbpath "%MONGO_DBPATH%" --no-ensure-account
)
