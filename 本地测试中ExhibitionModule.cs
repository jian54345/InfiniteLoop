    ...
    out RewardApplicationResult? result)
{
     ...
     * ★ 不使用 GiveRewards()
     *
     * 因为 GiveRewards() 会立即 SendPushes。
     *
     * 这里使用 ApplyRewards()，
     * 让 CharacterCommand 最后统一发送。
     */
    result = RewardHandler.ApplyRewards(
        rewardGoodsTables,
        session);
